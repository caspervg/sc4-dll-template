...
void HelloPanel::SetDetectedGameVersion(const uint16_t version) { detectedGameVersion_ = version; }
void HelloPanel::SetVersionLabel(const char* version) { versionLabel_ = version ? version : "dev"; }
void HelloPanel::SetVisible(const bool visible) { visible_ = visible; }
void HelloPanel::OnVisibleChanged(const bool visible) { visible_ = visible; }
